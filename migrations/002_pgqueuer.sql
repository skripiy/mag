-- Схема черги pgqueuer (таблиця pgqueuer, статистика, тригери NOTIFY).
-- DDL згенеровано командою `python -m pgqueuer install` (pgqueuer, версія з
-- pyproject) і зафіксовано як міграцію для детермінованого розгортання —
-- без залежності від конфігурації підключення CLI. Підрозділ 2.6/3.6.
-- При оновленні pgqueuer — перегенерувати цей файл.
...
CREATE TYPE pgqueuer_status AS ENUM ('queued', 'picked', 'successful', 'exception', 'canceled', 'deleted', 'failed');
    ...
    CREATE INDEX pgqueuer_log_not_aggregated ON pgqueuer_log (entrypoint, priority, status, created) WHERE not aggregated;
